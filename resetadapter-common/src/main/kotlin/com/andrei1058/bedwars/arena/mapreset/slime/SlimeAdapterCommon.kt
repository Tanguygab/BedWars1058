...
abstract class SlimeAdapterCommon<T>(owner: Plugin, plugin: String, displayName: String): RestoreAdapter<T>(owner, plugin, displayName) {
    ...
    override fun onEnable(arena: IArena) {
        if (api.versionSupport.mainLevel.equals(arena.worldName, ignoreCase = true) && (api.serverType != ServerType.BUNGEE || api.arenaManager.gamesBeforeRestart != 1)) {
            setMainLevel("ignore_main_level")
        ...
        run(async = true) {
            ...
            try {
                ...
                log.severe("Trying again to load arena: " + arena.name)
    ...
    fun setMainLevel(worldName: String) {
        val properties = Properties()

        try {
            FileInputStream("server.properties").use { properties.load(it) }
        } catch (e: IOException) {
            e.printStackTrace()
        }

        properties += mapOf(
            "level-name" to worldName,
            "generator-settings" to (if (api.versionSupport.version > 5) "minecraft:air;minecraft:air;minecraft:air" else "1;0;1"),
            "allow-nether" to "false",
            "level-type" to "flat",
            "generate-structures" to "false",
            "spawn-monsters" to "false",
            "max-world-size" to "1000",
            "spawn-animals" to "false",
        )

        try {
            FileOutputStream("server.properties").use { properties.store(it, null) }
        } catch (e: IOException) {
            e.printStackTrace()
        }
    }
