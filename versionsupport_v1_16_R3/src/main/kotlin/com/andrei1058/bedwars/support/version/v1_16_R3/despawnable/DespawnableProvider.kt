package com.andrei1058.bedwars.support.version.v1_16_R3.despawnable
...
abstract class DespawnableProvider<T: LivingEntity>(type: DespawnableType): DespawnableProviderCommon<T>(type) {
    ...
    private fun clearSelectors(entityLiving: EntityCreature) {
        try {
            PathfinderGoalSelector::class.java.getDeclaredField("d").apply {
                setAccessible(true)
                set(getGoalSelector(entityLiving), Sets.newLinkedHashSet<Any?>())
                set(getTargetSelector(entityLiving), Sets.newLinkedHashSet<Any?>())
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    override fun applyDefaultNMSSettings(bukkitEntity: LivingEntity, attr: DespawnableAttributes) {
        ...
        entity.getAttributeInstance(GenericAttributes.MAX_HEALTH)!!.value = attr.health
