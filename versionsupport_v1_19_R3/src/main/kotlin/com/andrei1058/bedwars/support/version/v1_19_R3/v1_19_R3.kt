...
@Suppress("ClassName")
class v1_19_R3(plugin: Plugin, name: String) : VersionSupportCommon(plugin, name, 9) {
    ...
    private fun getEntity(itemStack: ItemStack?) = itemStack?.nms?.G()
    ...
    override fun registerTntWhitelist(endStoneBlast: Float, glassBlast: Float) {
        ...
        coloredGlass.forEach { field.set(it, protection) }
    }
    ...
    override fun playRedStoneDot(player: Player) {
        ...
        player.world.players.forEach { if (it != player) it.nms.b.a(particlePacket) }
    }
